...
load Med_cons_SV_xhat_4P.mat
alpha = R.alpha;
beta  = R.beta;
%alpha = linspace(0.1,5,20);
%beta = linspace(20,50,60);
...
x0 = log(R.xhat);
%x0 = [b;r1;r2;r3;a;d];
%x0 = log([150;x0(2:end)./365]); % per day
nip = length(x0);
for jj = 1:length(X(:))

    p.alpha = X(jj);
    p.beta  = Y(jj);
    L = @(x) neglogpost_cons_SV_4P(x,p,grd,M2d);
    ...
    [f,dfdx,d2fdx2] = neglogpost_cons_SV_4P(xhat,p,grd,M2d);
    HH = d2fdx2;
...
%set(gca,'XTickLabel',{'0','0.10','0.20','0.30','0.40','0.50'})
%set(gca,'YTick',[250:25:350])
%set(gca,'YTickLabel',{'250','275','300','325','350'})
%text('Interpreter','latex','String','$$\Lambda = 0.23$$','Position',[1.5 230])
...
R.beta = p.beta;
R.logZ = logZ;
%fname = sprintf('Med_cons_SVa_xhat_4P');
%save(fname,'R');
