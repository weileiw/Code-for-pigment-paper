function [f,dfdx,d2fdx2] = neglogpost_cons_SV_log(x,p,grd,M2d)

nip    = length(x);
d2fdx2 = zeros(nip,nip);
dx = sqrt(-1)*eps.^3*eye(nip);

prior = [-7.05;-7.05;-7.05;-3.05];
U = d0([1/2.98;1/2.98;1/2.98;1/6.64]);
alpha = p.alpha;
beta  = p.beta;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%55
  ...
  e4 = log(chl_mod)-log(p.chl(2:end));
  e5 = log(Phy_mod)-log(p.Phyo(2:end));
  e6 = log(phy_mod)-log(p.phyo(2:end));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  de2dx = (1./poc_mod).*D(jj+1:2*jj,:)*dpdx;
  df2dx = (e2.'*W*de2dx);%/p.poc_std;

  de3dx = (1./Chl_mod).*D(2*jj+1:3*jj,:)*dpdx;
  df3dx = (e3.'*W*de3dx);%/p.Chl_std;

  de4dx = (1./chl_mod).*D(3*jj+1:4*jj,:)*dpdx;
  df4dx = (e4.'*W*de4dx);%/p.chl_std;

  de5dx = (1./Phy_mod).*D(4*jj+1:5*jj,:)*dpdx;
  df5dx = (e5.'*W*de5dx);%/p.Phyo_std;
...
%plot(log(real(M)),log(O),'*',log([1e-8:10]),log([1e-8:10]))
%xlim([1e-8,10]);
%ylim([1e-8,10]);
%r2 = rsquare(real(M),O);
...
%  D = -mfactor(FM,[dJBUdr1*N+dJBBdr1*M,...
%		   dJBUdr2*N+dJBBdr2*M,...
%		   dJBUdr3*N+dJBBdr3*M,...
%		   dJBUda*N+dJBBda*M,...
%		   dJBUdd*N+dJBBdd*M]);
